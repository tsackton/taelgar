---
headerVersion: 2023.11.25
lintedAt: "2026-08-21T10:48:34-04:00"
lintVersion: "3.4"
tags: [power, religion/tanshi]
typeOf: tanshi
name: Tera
pronunciation: TEH-rah
dm_owner: none
dm_notes: none
POV: modern
---
# Tera
*(TEH-rah)*
>[!info]+ Information  
> A [[Tanshi|tanshi]]

The tanshi of bare rock, rocky outcroppings, and high points of land, particularly those with views. Prefers offerings made from high, open points, when possible.

%%^Metadata:names:v1%%
- {name: Tera, role: primary, language: unknown, pronunciation: TEH-rah, status: documented}
%%^End%%

%%^povNotes:v1%%
Temporal coverage: broadly modern; the note describes Tera's enduring domain and customary offerings without a narrower dated state.
%%^End%%
