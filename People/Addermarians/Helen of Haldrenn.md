---
headerVersion: 2023.11.25
lintedAt: "2026-08-23T18:23:23-04:00"
lintVersion: "3.5"
tags: [person]
species: human
ancestry: Addermarian
campaignInfo:
  - {campaign: adma, type: met, date: 1715-05-17}
gender: female
born: 1662
name: Helen of Haldrenn
pronunciation: HEL-en of HAL-dren
affiliations:
  - {org: Haldrenn, title: temple administrator of the temple to the mother, type: member}
whereabouts:
  - {type: home, location: Haldrenn}
knownTo: [adma]
dm_owner: none
dm_notes: none
POV: 1715
---
# Helen of Haldrenn
*(HEL-en of HAL-dren)*
>[!info]+ Biographical Info  
> An [[Addermarch|Addermarian]] [[Humans|human]] (she/her)  
> `$=dv.view("_scripts/view/get_PageDatedValue")`  
> `$=dv.view("_scripts/view/get_Affiliations")`  
>> `$=dv.view("_scripts/view/get_Whereabouts")`  
>> %%^Campaign:adma%% Met by the [[Addermarch Mercenaries]] on May 17th, 1715 in [[Haldrenn]], [[Highmoor]], [[Carlinshire]] %%^End%%

Helen is the temple administrator for the small temple to [[The Mother]] in [[Haldrenn]], and a collector of fey lore. She presides over community rites and maintains oral histories of fey activity in the [[Highmoor]], including tales of the [[Morlaith|Briarheart]].
## Events
- (DR:: 1715-05-17): Presided over the funeral rites for [[Olay]], which were interrupted by an [[Addermarch - Session 12|worg attack]].

%% DM (PC interactions)
- Briefed the Addermarch Mercenaries on the Briarheart and presided over community rites: [[Addermarch - Session 11.md]]; [[Addermarch - Session 12.md]].
%%

%%^Metadata:names:v1%%
- {name: Helen of Haldrenn, language: Addermarian, pronunciation: HEL-en of HAL-dren, status: documented}
%%^End%%

%%^povNotes:v1%%
Temporal coverage: a DR 1715 portrait of Helen's religious and community role during the fey disturbances around Haldrenn.
%%^End%%
