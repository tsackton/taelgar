---
headerVersion: 2023.11.25
lintedAt: "2026-08-23T11:35:40-04:00"
lintVersion: "3.5"
tags: [person]
species: human
ancestry: Addermarian
campaignInfo:
  - {campaign: adma, type: met, date: 1715-05-11}
gender: female
name: Samantha of Fontwick
whereabouts:
  - {type: home, location: Fontwick}
knownTo: [adma]
dm_owner: none
dm_notes: none
POV: 1715
---
# Samantha of Fontwick
>[!info]+ Biographical Info  
> An [[Addermarch|Addermarian]] [[Humans|human]] (she/her)  
>> `$=dv.view("_scripts/view/get_Whereabouts")`  
>> %%^Campaign:adma%% Met by the [[Addermarch Mercenaries]] on May 11th, 1715 in [[Fontwick]], the [[Stonewolds]], [[Carlinshire]] %%^End%%

Samantha is a villager from [[Fontwick]], a small village west of [[Roscombe]]. She operates a small dairy alongside her husband [[Godfrey of Fontwick]]. She is practical, hospitable, and resilient.

## Events
- (DR:: 1715-05-11): Encountered the [[Addermarch Mercenaries]] while fleeing an ankheg attack on her cattle, and led them back to the farm.
- (DR:: 1715-05-12): Gratefully received funds from [[Yvan Greenrabbit]] to restock her herd, and provided the party with food for the road. 

%% DM (PC interactions and sources)

 DM Notes: [[Adventure 4 - Into the Underdark]]
 
%%

%%^Metadata:names:v1%%
- {name: Samantha of Fontwick, language: Addermarian, status: inferred}
%%^End%%

%%^povNotes:v1%%
Temporal coverage: a DR 1715 portrait of Samantha as a Fontwick dairy farmer during and immediately after the ankheg attack on her herd; earlier and later life are not described.
%%^End%%
