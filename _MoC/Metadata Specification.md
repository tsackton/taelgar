...
- `dm_notes`: Human attestation indicating whether useful private, non-shared DM information exists. Acceptable values: `important`, `color`, `none`; linting does not adjudicate between the two positive values or automatically change an attestation. The contextual lint review uses matching `_DM_` Markdown notes: `none` plus a genuine match with plausible recoverable material receives a private chat-only summary and candidate addition, a positive value plus no matches receives a verification suggestion, and other combinations need no user-facing source list. When an independently open note records recoverable `_DM_` source paths, each exact wikilink is a separate dash-bulleted Markdown list item and no private content enters the Git-shared report; otherwise missing links appear only in the chat handoff, without duplication. Once reviewed under the adopted `dmNotesReviewVersion`, `lintedAt` validates the attestation until a matching `_DM_` file is modified later or the minimum review version is raised. In-note `%%SECRET[v2:2813636d58fe60b6f07f9b3fae26e409]%%`, ordinary comments, and `Campaign:none` blocks never count as positive external evidence and never affect the candidate-based review. `SECRET` blocks are nevertheless eligible for the same private chat-only recovery handoff. See: [[Note Status]] and [[Taelgar Note Linter]].
...
Purpose: track which campaign parties have met or otherwise meaningfully interacted with the subject (usually a person), in a way that:
- supports simple frontmatter queries (“who has the party interacted with?”), and
- avoids cluttering the YAML/frontmatter with long interaction logs.
