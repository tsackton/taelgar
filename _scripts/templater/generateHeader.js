async function generateHeader(tp) {
    ...
    if (tp.frontmatter.type == "NPC" || tp.frontmatter.type == "Ruler") {
        ...
        if (tp.frontmatter.type == "NPC") {
            ...
            if (tp.frontmatter.family) {
                headerString += "> #### Family\n";
                headerString += ">> [[" + tp.frontmatter.family + "]]\n";
                if (tp.frontmatter.parents) {
                    tp.frontmatter.parents.forEach(element => {
                        headerString += ">> Parent: [[" + element + "]]\n";
                    });
                }
                if (tp.frontmatter.children) {
                    tp.frontmatter.children.forEach(element => {
                        headerString += ">> Issue: [[" + element + "]]\n";
                    });
                }
            }

            if (tp.frontmatter.kingdom) {
                headerString += "> #### " + tp.frontmatter.title + " of " + tp.frontmatter.kingdom + "\n"
                headerString += '>> `$=dv.view("' + dViewPath + 'get_RegnalValue", {"currentYear" : (dv.current().yearOverride ? ' +
                    'dv.current().yearOverride : FantasyCalendarAPI.getCalendars()[0].current.year)})`\n';
                if (tp.frontmatter.predecessor) {
                    headerString += ">> Predecessor: [[" + tp.frontmatter.predecessor + "]]\n"
                }
                if (tp.frontmatter.successor) {
                    headerString += ">> Successor: [[" + tp.frontmatter.successor + "]]\n"
                }
            }
        }
