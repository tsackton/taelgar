function get_Whereabouts(metadata, input) {
    ...
    if (metadata.whereabouts) {
        ...
        let bornDate = Date.UTC(1, 0, 0, 0, 0, 0, 0);
        if (metadata.born) {
            bornDate = Date.UTC(metadata.born, 0, 0, 0, 0, 0, 0);
        }
        ...
        let current = metadata.whereabouts.findLast(s => s.date <= currentJsDate);
        let home = metadata.whereabouts.findLast(s => s.date <= currentJsDate && !s.excursion);
        let original = metadata.whereabouts.find(s => s.date >= bornDate);
