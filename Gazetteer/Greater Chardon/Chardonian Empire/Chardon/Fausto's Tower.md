...
Fausto's Tower is the residence and workshop of [[Fausto]], a wealthy Chardonian enchanter and patron of adventurers associated with the [[Society of the Open Scroll]].

%% Extensive DM notes elsewhere %%

%%^Metadata:names:v1%%
- {name: "Fausto's Tower", role: primary, language: Chardonian, pronunciation: FOW-stohz TOW-er, status: proposed, notes: "Derived from Fausto's accepted pronunciation in [[Fausto]] plus the ordinary English possessive and title."}
%%^End%%

%%^povNotes:v1%%
Temporal coverage: a pre-defeat snapshot through early DR 1749, before Fausto's defeat and the tower's later guarded occupation.
%%^End%%
...
### Applied changes
- Reassessed and migrated the legacy article metadata to `povNotes:v1`, retaining the DR 1748 pre-defeat snapshot.
- Added a proposed `Metadata:names:v1` entry using Fausto's accepted pronunciation and the ordinary English possessive and title.
...
### Status dispositions
- `status/gameupdate/dufr` is not assessable until the coverage choice below is resolved; it was preserved unchanged.
