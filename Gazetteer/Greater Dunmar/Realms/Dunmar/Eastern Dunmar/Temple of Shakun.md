---
headerVersion: 2023.11.25
lintedAt: "2026-08-21T19:01:27-04:00"
lintVersion: "3.4"
displayDefaults: {aNoDate: "<affiliationtitle:tx> to <affiliations>"}
tags: [place]
typeOf: holy site
typeOfAlias: temple
ancestry: Dunmari
name: Temple of Shakun
pronunciation: "SHAH-koon"
affiliations:
  - {org: Shakun, title: Dedicated}
whereabouts: Karawa
dm_owner: tim
dm_notes: important
POV: 1740s
---
# The Temple of Shakun
*(SHAH-koon)*
>[!info]+ Information  
> `$=dv.view("_scripts/view/get_Affiliations")`  
>> `$=dv.view("_scripts/view/get_Whereabouts")`

%%TIM: a lot of design notes and possibly a map somewhere in my DM notes %%

A large, imposing structure carved out of the side of the red sandstone mesa on the southern part of town. The temple itself stretches at least partially into the cliff itself, but you have never been in that part of the structure. Non-initiates are only allowed access to a few outbuildings, and to the chapel to Shakun facing the oasis. The chapel is beautiful - polished sandstone floors, and tile mosaics abound - but is a place primarily for prayer and does not seem to hold any secrets. The [[Shakun Mystai|Mystai of Shakun]] do, however, sell and barter dusts and powders that are known to possess magical properties. The temple, strangely, seems more weathered and aged than the rock of the mesa itself.

%%^Campaign:dufr%%
This sanctuary is also the hidden seat of the [[Shakun Mystai]] and contains the sacred [[Heart of Shakun]]. The Heart's presence sustains the mesa's living stone and supports [[Shakun's Protection]] along Dunmar's eastern frontier.
%%^End%%

%%^Metadata:names:v1%%
- {name: "Temple of Shakun", role: "primary", language: "Dunmari", pronunciation: "SHAH-koon", status: "documented"}
%%^End%%

%%^povNotes:v1%%
Temporal coverage: a DR 1740s description of the Temple of Shakun as experienced by non-initiates; its restricted interior and deeper sacred chambers are outside the visible account.
%%^End%%