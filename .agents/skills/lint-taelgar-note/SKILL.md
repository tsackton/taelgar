...
- identity, classification, aliases, strict actual-pronunciation disposition, and contextually applicable human-curated name metadata;
...
- Choose `POV` only after isolating narrower dated state. If one supported event or change is narrower than the rest of the article, wrap the smallest affected passage in a copy-ready `Date:*` block and keep the broadest honest whole-note POV. Do not call a decade- or era-generic article a single-year snapshot merely because it contains one dated paragraph.
