...
- searchable frontmatter `POV`, article mode, and a concise note-specific `povNotes` temporal-coverage explanation; every completed write-mode lint records the least precise useful reading position supported by the undated visible article frame;
...
- Choose `POV` from the undated visible article frame, not from the span between its oldest and newest facts. Use `modern` when no finer position matters within the DR 1700s campaign era, a decade when rough campaign era or life stage matters, and a year for a narrower snapshot. Historical backstory does not widen `POV`; an isolated dated event does not narrow it. Reconsider legacy `timeless`, named-era, and century values contextually on re-lint rather than replacing them mechanically.
- Write `povNotes` beginning with `Temporal coverage:` and keep it concise, normally one sentence and at most two when needed. Describe the coverage as broad or approximately bounded, narrow, one-sided uncertain, or discontinuous; name established before/after events when useful, distinguish unknown periods from contradicted states, and never invent an exact cutoff or infer continuity between separated facts. Do not restate every lifecycle date, dated metadata value, or `Date:*` block unless it changes how the article should be read.
- Isolate narrower dated state before finalizing `POV`. If one supported event or change is narrower than the rest of the article, wrap the smallest affected passage in a copy-ready `Date:*` block and keep the broader whole-note POV. Do not call a decade- or modern-generic article a single-year snapshot merely because it contains one dated paragraph.
...
- Treat name metadata as human-curated. When [[Languages]] documents a real-world analogue for the inferred or established language, always generate the pronunciation by asking how that spelling would naturally be pronounced or adapted in the analogue language. Lack of exact in-world phonology changes the result's status to `proposed`; it is never a reason to fall back to English pronunciation. Explain the concrete sound choices—such as consonant values, digraphs, vowels, and stress—and identify the documented analogue. If multiple analogues produce materially different readings, give a preferred proposal and explain the alternatives. Use a merely spelling-based reading only when no stronger language rule, pattern, or analogue exists. Persist an unaccepted pronunciation in the name block with `status: proposed`; never leave its only copy in the replaceable Lint block or promote it to frontmatter before human acceptance.
- Give every note whose subject is a named in-world thing or work a `Metadata:names:v1` block. Apply this rule semantically, not from the note's tag, folder, or title alone. Notes about people, places, objects, groups, events, powers, creatures, ancestries or cultures, religions, primary-source works, and other named in-world subjects receive the block. A meta or background page that organizes, analyzes, or summarizes information without itself describing an in-world subject does not; for example, `History of Sembara`, `Timeline of Tollen`, and `West Coast History Framework` do not receive blocks merely because their titles contain in-world names. A `background`-tagged note that actually describes a named in-world subject does receive one. When no stronger name facts are known, write the exact subject name with `language: unknown`; do not invent etymology. A plain-English or otherwise obvious name may omit pronunciation, but it does not omit the required name block.
