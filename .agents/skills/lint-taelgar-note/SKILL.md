...
- For required map metadata with unknown positions, insert the validator's typed skeleton instead of `status: missing` and `locations: []`. Waterways use `source` and `outlet` point entries with blank `feature` and `locator`; roads use one world-map path with blank `sourceHex` and `outletHex`; settlements use one world-map point with a blank `locator`. Preserve the blank YAML values until a human supplies the positions, and keep `metadata.map_location_missing` open. Replace legacy empty placeholders with the typed skeleton during re-linting.
