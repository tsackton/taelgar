...
Each editorial worker reads every assigned note completely, uses its packet plus one integrated contextual evidence pass, and applies supported lint-owned changes only to the staged copy while preserving its old completion state, tag, and report. It sets semantic eligibility; for an eligible note it also sets exactly one editorial verdict, a clean or open outcome, a complete replacement `%%^Lint%%` block when open, structured `_DM_`, `SECRET`, and shared-nonpublic dispositions, declared objective body edits, an explicit assessment when underdeveloped, and a structured expansion candidate when worth expanding. Disposition every manifest `_DM_` source as `matching` or `not_matching` with a concise noncontent identity summary. For a confirmed match when the target has `dm_notes: none`, also classify whether it offers no recoverable material, a plausible public addition, or a plausible private addition. A plausible addition requires a content-level chat summary and bounded copy-ready candidate. For other `dm_notes` values and for rejected matches, record recovery as not applicable. Independently disposition every `SECRET` block for chat-only recovery, with the same public/private distinction and a copy-ready candidate when useful. An ineligible result needs a concise reason and no staged change, verdict, outcome, report, handoff, or review payload. After the staged candidate and result are final, set `candidateSha256` in that result to the candidate's exact SHA-256; any later candidate mutation invalidates the result.

The result's `handoff` is private review material and may contain content from local `_DM_` notes or `SECRET` blocks. For each plausible recovery, it includes the exact marker `Destination: public` or `Destination: private`, states what the private source adds, and gives the copy-ready candidate. Never copy, quote, or paraphrase that content into `lintReport`; the finalizer blocks recorded private text and raw source excerpts, while the editorial worker remains responsible for detecting paraphrased disclosure. If the final note is already open, record only the corresponding `_DM_` source links in the replacement Lint block, each as its own dash-bulleted Markdown list item, and omit those links from the chat handoff. Every `_DM_` wikilink the linter writes into any Lint block follows this one-link-per-list-item format; never put the links inline or combine several on one item. If the final note is clean and does not already contain those links as dash-bulleted list items outside an old Lint block, list them in the chat handoff. Do not duplicate links between note and chat. Do not list or summarize merely matching, redundant, or unhelpful sources, and do not require this recovery handoff for `dm_notes: color` or `important`.
...
5. **Privacy:** disposition every substantive ordinary comment and `Campaign:none` block as `redundant_with_public`, `public_adoption_candidate`, `dm_only`, `speculative_or_unresolved`, `source_pointer`, or `no_useful_material`. A public adoption candidate requires open `editorial.public_material_candidate`; material redundant with visible prose requires open `editorial.shared_material_redundant`. Neither disposition can be clean or merely worth-expanding. Preserve the hidden text and propose human action; never promote or remove it automatically. Never copy or paraphrase `SECRET` or local `_DM_` content into a Git-shared report. The private chat handoff may share that content with the user and, when `dm_notes: none` has a matching source or a `SECRET` block contains plausible recoverable material, must explain the addition and provide a public- or private-destination candidate. Use validator or manifest `_DM_` matches for the gated `dm_notes` review: matches do not invalidate `dm_notes: none`, and no match does not authorize clearing a positive human attestation.
...
State each note's editorial sufficiency verdict in the chat handoff, along with which files changed, whether each result is clean or open, what human work remains, and what validation passed. For worth-expanding notes, render the structured addition concretely and name its source notes while preserving reported, assumed, provisional, or uninvented status. For each `_DM_` or `SECRET` recovery candidate, render the private result as a separate chat-only item with proposed destination, content-level summary, and copy-ready addition. Include exact `_DM_` wikilinks as Markdown list items only when they are not already listed in the finalized note; never print bare counts as a substitute. Omit nonrecoverable matches and omit the recovery section entirely when nothing useful was found. Shared-nonpublic dispositions follow their ordinary Git-shared rules; DM-only contents from those shared blocks are not reproduced unless the user separately requests them. Write an Underdeveloped assessment into its open Lint report; other sufficiency verdicts remain chat-only. If the skill or tooling changed, run its focused tests and the skill structural validator.
