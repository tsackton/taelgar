...
This skill stops before `session-note-prep`; its job is to create or update the source prep YAML and speaker mapping JSON, show them to the user for verification, and then run `python .agents/skills/prepare-session-source/scripts/prepare_source.py` only after explicit approval.
...
The `.m4a` recording with the same stem is inferred by `prepare_source.py` and will become `sourceAudioPath` in the manifest; add `sourceAudioPath` to the config only when inference is not obvious or the user wants an explicit path.
...
Starter files are available in [assets/source_prep_config.template.yaml](assets/source_prep_config.template.yaml) and [assets/participants.template.yaml](assets/participants.template.yaml).
...
For transcript sessions, `prepare_source.py` requires exactly one of:
...
Before running `prepare_source.py`, print both full artifacts for review:
...
Run from the Taelgar vault root using a Python environment with PyYAML installed.
WebVTT input also requires `webvtt-py`:

```bash
cd "/Users/tim/Library/Mobile Documents/iCloud~md~obsidian/Documents/Taelgar"
python .agents/skills/prepare-session-source/scripts/prepare_source.py \
...
For `campaign: Lost in the Feywild` and `sessionNumber: 4`, `prepare_source.py` writes:
