...
Subject: Taelgar post-session bit

	Including Mike on message because this is related to plot so he should know what’s been conveyed.
	It occurs to me that this isn’t so much a conversation that Celyn and Robin need to have as a low-level meltdown that Celyn has that Dan needs to be aware of so no response required unless brain produces dialogue/reaction; it or something very like ought to happen sometime in the next play session so it seemed reasonably sensible to write it up and then when we know when there’s an opportunity for something like this to happen it can be inserted into the narrative accordingly.
	Partly writing this because Dan wasn’t there for the end of the session where the information came out and thus conveying it in character is more interesting than just writing a summary of it; partly writing because barring utter disaster this bit would fall sometime close after the rescue attempt and better to both be able to say ‘that side conversation happens here’ and carry on with session and to have the information in player knowledge when it would be known.
	...
	So presumably we have whatever happens in the first part of next session and, assuming we don’t die or are otherwise deeply thwarted, at some point there can be this bit, when/where it seems safe enough to actually talk about such things and there isn’t an immediate crisis preventing it like the rescue attempt:
