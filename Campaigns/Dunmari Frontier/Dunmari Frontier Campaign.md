...
You can also explore the most recent session, [[Session 98 (DuFr)]] or browse pages mentioned in the most recent session note:

```dataview
TABLE WITHOUT ID Link, join(split(meta(Link).path,"/",2),"/") as Folder
FROM "Campaigns/Dunmari Frontier/Session Notes/Session 98 (DuFr)"
