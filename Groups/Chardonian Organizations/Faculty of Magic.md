---
headerVersion: 2023.11.25
lintedAt: "2026-08-23T12:50:50-04:00"
lintVersion: "3.5"
displayDefaults: {boxInfo: "<ancestry:UA> <typeof:UA> of <partof:UA>", partOf: ""}
tags: [group, status/gameupdate/dufr]
typeOf: faculty
partOf: University of Chardon
name: Faculty of Magic
whereabouts: Chardon
dm_owner: tim
dm_notes: none
POV: 1748
---
# The Faculty of Magic
>[!info]+ Information  
> A faculty of the [[University of Chardon]]  
> `$=dv.view("_scripts/view/get_Affiliations")`  
>> `$=dv.view("_scripts/view/get_Whereabouts")`

The Faculty of Magic is one of the five graduate faculties of the [[University of Chardon]]. Its masters train practitioners of the arcane arts, and their teachings shape much of Chardon’s reputation as a city of wizards and scholars.

The Faculty of Magic is intertwined with other institutions of the academic quarter. Many of its members also participate in organizations such as the [[Society of the Open Scroll]], and the university as a whole is closely affiliated with the [[Great Library]] and the [[Hetaeri Magica]].

%%SECRET[v2:082c998ee5be3ea82217bfb3470a1b8b]%%

%%^Metadata:names:v1%%
- {name: Faculty of Magic, role: primary, language: Common, status: documented, notes: Plain descriptive institutional title.}
%%^End%%

%%^povNotes:v1%%
Temporal coverage: a DR 1748 institutional overview before the fallout from Fausto's defeat and the DR 1749 Chardon chalyte crisis.
%%^End%%
